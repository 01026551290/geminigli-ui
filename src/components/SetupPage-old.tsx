...
export default function SetupPage({ onSetupComplete }: SetupPageProps) {
  const [cliStatus, setCliStatus] = useState<'checking' | 'found' | 'not-found'>('checking');
  const [apiKeyStatus, setApiKeyStatus] = useState<'checking' | 'found' | 'not-found' | 'input'>('checking');
  const [apiKey, setApiKey] = useState('');
  const [isInstalling, setIsInstalling] = useState(false);
  const [installProgress, setInstallProgress] = useState('');
  const [isValidatingKey, setIsValidatingKey] = useState(false);
  const [keyValidation, setKeyValidation] = useState<{status: 'idle' | 'valid' | 'invalid', message?: string}>({status: 'idle'});
  ...
  const checkGeminiCli = async (): Promise<boolean> => {
    try {
      const result = await Command.create("sh", ["-c", "which gemini"]).execute();
      if (result.code === 0) {
        setCliStatus('found');
        return true;
      } else {
  ...
  const checkExistingApiKey = async () => {
    ...
    try {
      ...
      const result = await Command.create("sh", [
        "-c", 
        `echo "test" | timeout 10 /opt/homebrew/bin/gemini --model gemini-1.5-flash 2>&1 | head -1`
      ]).execute();
      ...
      if (result.code === 0 && result.stdout && 
        ...
        setKeyValidation({status: 'valid', message: '기존 API 키가 설정되어 있습니다'});
      } else {
  ...
  const validateApiKey = async (key: string): Promise<boolean> => {
    if (!key || key.length < 30) {
      setKeyValidation({status: 'invalid', message: 'API 키가 너무 짧습니다'});
      return false;
    }

    if (!key.startsWith('AI')) {
      setKeyValidation({status: 'invalid', message: 'Google AI Studio API 키는 "AI"로 시작해야 합니다'});
      return false;
    }
    ...
    try {
      ...
      const testCommand = `echo "Hello" | timeout 10 GEMINI_API_KEY="${key}" /opt/homebrew/bin/gemini --model gemini-1.5-flash`;
      const result = await Command.create("sh", ["-c", testCommand]).execute();
      
      if (result.code === 0 && result.stdout && !result.stdout.toLowerCase().includes('error')) {
        ...
        setKeyValidation({status: 'invalid', message: 'API 키가 유효하지 않습니다'});
        return false;
      }
    } catch (error) {
      console.error("API 키 검증 오류:", error);
      setKeyValidation({status: 'invalid', message: 'API 키 검증 중 오류가 발생했습니다'});
      return false;
    } finally {
  ...
  const installGeminiCli = async () => {
    ...
    try {
      const brewCheck = await Command.create("sh", ["-c", "which brew"]).execute();
      ...
      const geminiInstall = await Command.create("sh", ["-c", "brew install gemini"]).execute();
  ...
  const openGoogleAiStudio = () => {
    if (typeof window !== "undefined" && "__TAURI_INTERNALS__" in window) {
      import("@tauri-apps/plugin-shell").then(({ Command }) => {
        Command.create("sh", ["-c", "open https://aistudio.google.com/apikey"]).execute();
      });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-lg shadow-lg p-8 max-w-lg w-full">
        <h1 className="text-2xl font-bold text-center mb-8">Gemini Desktop 설정</h1>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            {cliStatus === 'found' && <Check className="text-green-500" size={20} />}
            {cliStatus === 'not-found' && <X className="text-red-500" size={20} />}
          </div>
          
          {cliStatus === 'not-found' && (
            <div className="bg-red-50 border border-red-200 rounded p-4">
              <p className="text-red-700 mb-3">Gemini CLI가 설치되어 있지 않습니다.</p>
              {isInstalling ? (
                <div>
                  <p className="text-sm text-gray-600 mb-2">{installProgress}</p>
          ...
          {cliStatus === 'found' && (
            <div className="bg-green-50 border border-green-200 rounded p-4">
              <p className="text-green-700">✅ Gemini CLI가 설치되어 있습니다.</p>
            </div>
          )}
        </div>
        ...
        {cliStatus === 'found' && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              {apiKeyStatus === 'found' && <Check className="text-green-500" size={20} />}
              {apiKeyStatus === 'not-found' && <X className="text-red-500" size={20} />}
            </div>

            {apiKeyStatus === 'checking' && (
              <div className="bg-blue-50 border border-blue-200 rounded p-4">
                <p className="text-blue-700">기존 API 키 설정을 확인하는 중...</p>
              </div>
            )}
            ...
            {apiKeyStatus === 'found' && (
              <div className="bg-green-50 border border-green-200 rounded p-4">
                <div className="flex items-center space-x-2 text-green-700 mb-3">
                  <Check size={16} />
                  <span className="font-medium">API 키가 이미 설정되어 있습니다!</span>
                </div>
                ...
                <div>
                  ...
                  {keyValidation.message && (
                    <div className={`flex items-center space-x-2 mt-2 text-sm ${
                      keyValidation.status === 'valid' ? 'text-green-600' : 'text-red-600'
                    }`}>
